...
class Regex {
    public static void main(String[] args) {
        ...
        System.out.println("The following pattern ('\\d'){36} matches : any 36 digits in a row");
        ...
        Pattern p = Pattern.compile("(Amir|Mark|Rosa|Desaree|Abram|Dylan|Demilios|Jonathan)");
        List<String> matches = new ArrayList<String>();
        for (int i = 0; i < TEKmentors.length; i++) {
            if (TEKmentors[i].matches("(Amir|Mark|Rosa|Desaree|Abram|Dylan|Demilios|Jonathan)")) { // matches uses regex
        ...
        String[] noSleepy = Pattern.compile("^((?!sleepy).)*$")
                .matcher(sleepy)
                .results()
